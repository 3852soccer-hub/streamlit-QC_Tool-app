...
def pareto_chart(df: pd.DataFrame, title: str = "パレート図") -> PngFigure:
...
def histogram_with_normal(series: pd.Series, bins: int = 20, title: str = "ヒストグラム") -> PngFigure:
    ...
    if sigma > 0:
        ...
        ax.plot(x, y, color=sns.color_palette()[3], linewidth=2, label="正規分布(推定)")
...
def before_after_distribution_plot(
    ...
    # densityで比較
    ax.hist(b, bins=bins, density=True, alpha=0.55, color=sns.color_palette()[0], label=f"改善前 (n={len(b)})")
    ax.hist(a, bins=bins, density=True, alpha=0.55, color=sns.color_palette()[3], label=f"改善後 (n={len(a)})")
...
def doe_main_effects_plot(main_effects_long: pd.DataFrame, title: str = "DOE 主効果プロット") -> PngFigure:
...
def xr_control_chart_plot(xr: capability_engine.XRChartResult, title: str = "X-R管理図") -> PngFigure:
    ...
    ax1.set_ylabel("\u0305X")
    ax1.set_title(f"{title}（サブグループn={xr.subgroup_size}）")
...
def ppk_distribution_plot(
    ...
    if sigma_overall > 0:
        ...
        ax.plot(x, y, color=sns.color_palette()[3], linewidth=2, label="正規分布(σ_overall)")

    ax.axvline(lsl, color="red", linestyle="--", linewidth=1.5, label="LSL")
    ax.axvline(usl, color="red", linestyle="--", linewidth=1.5, label="USL")
    ax.axvline(mean, color="black", linewidth=1.2, label="平均")
